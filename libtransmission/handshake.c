...
static void gotError(tr_peerIo* io, short what, void* vhandshake)
{
    ...
    dbgmsg(handshake, "libevent got an error what==%d, errno=%d (%s%s)", (int)what, errno, tr_strerror(errno), (what & BEV_EVENT_EOF) ? " eof " : "");
...
tr_handshake* tr_handshakeNew(tr_peerIo* io, tr_encryption_mode encryptionMode, handshakeDoneCB doneCB, void* doneUserData)
{
    ...
    dbgmsg(handshake, "%s %s handshake requested.", tr_peerIoIsIncoming(handshake->io) ? "Incoming" : "Outgoing", io->socket.type == TR_PEER_SOCKET_TYPE_UTP ? "utp" : "tcp");
